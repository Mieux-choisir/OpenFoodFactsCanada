...
def calculate_similarity(row):
    fields_to_compare = [col for col in row.index if "_src1" in col and "_id" not in col]
...
def main():
    ...
    for _, row in merged_df.iterrows():
        entry = {"id": row.name, "similarity": row["similarity"], "matches": {}, "differences": {}}
        ...
        for col in merged_df.columns:
            if "_src1" in col and "_id" not in col:
                ...
                if similarity >= 90:
                    ...
                    entry["differences"][col_pair] = {"src1": col1, "src2": col2, "similarity": f"{similarity:.2f}%"}
