...
@pytest.fixture
def given_fdc_dict():
    ids = {"fat": 1004, "sodium": 1093, "saturated_fats": 1258, "sugar": 2000, "carbohydrates": 1005, "energy_kcal":1008, "vitamin_a": 1104}

    fdc_dict = [{"nutrient": {"id": ids["fat"], "unitName": "g"}, "amount": 10},
...
def test_should_return_mapped_nutrient_level_values_in_nutrition_facts_for_fdc_dict(nutrition_facts_mapper, given_fdc_dict):
    _, fdc_dict = given_fdc_dict

    with patch.object(NutrientMapper,
                      "map_nutrient",
                      return_value=25):
...
def test_should_return_mapped_nutrients_in_nutrition_facts_for_fdc_dict(nutrition_facts_mapper, given_fdc_dict):
    _, fdc_dict = given_fdc_dict
...
def test_should_return_correct_energy_values_for_nutrients_in_nutrition_facts_for_fdc_dict(nutrition_facts_mapper, given_fdc_dict):
    ...
    expected_energy_kcal_100g = next(item["amount"] for item in fdc_dict if item["nutrient"]["id"] == fdc_ids["energy_kcal"])
    assert result.nutrients.energy_100g == float(expected_energy_kcal_100g * Decimal(4.1868))
