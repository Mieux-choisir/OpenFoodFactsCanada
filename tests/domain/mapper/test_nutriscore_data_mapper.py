...
def fdc_dict():
    ...
    nutrient_values = {"energy": 45.2, "fibers": 54, "proteins": 20, "saturated_fats": 63.78, "sodium": 0.5, "sugar": 45.14}

    food_nutrients = [{"nutrient": {"number": nutrient_values["energy"], "id": energy_id}},
                {"nutrient": {"number": nutrient_values["fibers"], "id": fibers_id}},
                {"nutrient": {"number": nutrient_values["proteins"], "id": proteins_id}},
...
@pytest.fixture
def off_valid_rows():
    header = ["nutriscore_grade", "energy_100g", "fiber_100g", "fruits-vegetables-nuts_100g", "proteins_100g", "saturated-fat_100g", "sodium_100g", "sugars_100g"]
...
def test_should_return_empty_score_in_nutriscore_data_for_given_fdc_dict(nutriscore_data_mapper):
    food_nutrients = []
...
def test_should_assign_given_nutrient_values_in_nutriscore_data_for_given_fdc_dict(nutriscore_data_mapper, fdc_dict):
...
def test_should_return_empty_fruit_percentage_in_nutriscore_data_for_given_fdc_dict(nutriscore_data_mapper, fdc_dict):
    food_nutrients, nutrient_values = fdc_dict
...
def test_should_return_empty_is_beverage_field_in_nutriscore_data_for_given_fdc_dict(nutriscore_data_mapper, fdc_dict):
    food_nutrients, nutrient_values = fdc_dict
...
def test_should_return_score_from_number_mapper_for_given_score_in_nutriscore_data_for_given_off_row(nutriscore_data_mapper, off_valid_rows):
...
def test_should_return_empty_score_for_absent_score_in_nutriscore_data_for_given_off_row(nutriscore_data_mapper, off_invalid_rows):
...
def test_should_assign_given_valid_nutrient_values_in_nutriscore_data_for_given_off_row(nutriscore_data_mapper, off_valid_rows):
    row, header = off_valid_rows

    result = nutriscore_data_mapper.map_off_row_to_nutriscore_data(row, header)

    assert (
            result.energy == float(row[header.index("energy_100g")])
    ), f"Expected energy value to be {row[header.index("energy_100g")]}, got {result.energy}"
    assert (
            result.fibers == float(row[header.index("fiber_100g")])
    ), f"Expected energy value to be {row[header.index("fiber_100g")]}, got {result.fibers}"
    assert (
            result.fruit_percentage == float(row[header.index("fruits-vegetables-nuts_100g")])
    ), f"Expected energy value to be {row[header.index("fruits-vegetables-nuts_100g")]}, got {result.fruit_percentage}"
    assert (
            result.proteins == float(row[header.index("proteins_100g")])
    ), f"Expected energy value to be {row[header.index("proteins_100g")]}, got {result.proteins}"
    assert (
            result.saturated_fats == float(row[header.index("saturated-fat_100g")])
    ), f"Expected energy value to be {row[header.index("saturated-fat_100g")]}, got {result.saturated_fats}"
    assert (
            result.sodium == float(row[header.index("sodium_100g")])
    ), f"Expected energy value to be {row[header.index("sodium_100g")]}, got {result.sodium}"
    assert (
            result.sugar == float(row[header.index("sugars_100g")])
    ), f"Expected energy value to be {row[header.index("sugars_100g")]}, got {result.sugar}"


def test_should_return_empty_nutrient_values_for_invalid_values_in_nutriscore_data_for_given_off_row(nutriscore_data_mapper, off_invalid_rows):
    row, header = off_invalid_rows
...
def test_should_return_empty_is_beverage_field_in_nutriscore_data_for_given_off_row(nutriscore_data_mapper, off_valid_rows):
    row, header = off_valid_rows
