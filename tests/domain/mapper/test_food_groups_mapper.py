...
def test_should_return_correctly_split_food_groups_list_for_given_off_row(
    food_groups_mapper, off_rows
):
    row, header = off_rows
    ...
    expected_result = [
        x.strip()
        for x in row[header.index("food_groups_en")].lower().split(",")
        if x.strip() != ""
    ]
    assert (
...
def test_should_return_empty_food_groups_list_for_given_off_row(
    food_groups_mapper, off_empty_rows
):
